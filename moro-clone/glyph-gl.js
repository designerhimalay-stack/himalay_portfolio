...
(function () {
  'use strict';
  var cv = document.getElementById('glyphgl');
  if (!cv) return;
...
'void main(){',
// split the canvas into cells; each pixel only ever marches its own glyph.
// one row of four on a wide screen, two-over-two when narrow — four across a
// phone leaves the marks touching. uv is normalised by CELL height, which makes the
// lens independent of the grid: a glyph is always the same size in its cell.
'  float ch=R.y/CY;',
'  float cy=floor((R.y-gl_FragCoord.y)/ch);',
'  float ncol=(CY<1.5)?4.0:2.0;',
'  float base=(CY<1.5)?0.0:cy*2.0;',
'  float cw=R.x/ncol;',
'  float cx=floor(gl_FragCoord.x/cw);',
'  float gi=base+cx;',
'  if(gi>3.5){gl_FragColor=vec4(0.);return;}',
'  vec2 cellO=vec2((cx+0.5)*cw, R.y-(cy+0.5)*ch);',
'  vec2 uv=(gl_FragCoord.xy-cellO)/ch;',
...
'  gl_FragColor=vec4(col,1.0);}'
  ...
  var uR = gl.getUniformLocation(pr,'R'), uT = gl.getUniformLocation(pr,'T'),
      ...
      uCY = gl.getUniformLocation(pr,'CY');

  /* the hero keeps the blue; these are the same five hues the headline hover and
     the nav dots already use, so the palette is one system, not three. */
  ...
  /* the field has to be baked from the loaded face, not the fallback */
  function loadAmp() {
    var f = bakeField('&', '500 160px "Instrument Sans", sans-serif');
    if (!f) return;
  ...
  /* The exact 1024x1024 transform blocks the main thread for ~150ms. Run it at
     load and it lands inside the hero's arrival and stutters it. The section is
     below the fold, so it is baked the first time the row comes near the
     viewport instead — by then the arrival is long finished. */
  var ampDone = false;
  function ampReady() {
    if (ampDone) return;
    ampDone = true;
  ...
  function frame(now) {
    ...
    if (running || resized) {
      ...
      gl.uniform1f(uF, 3.4);
      gl.uniform1f(uCY, cv.clientWidth < 720 ? 2.0 : 1.0);
